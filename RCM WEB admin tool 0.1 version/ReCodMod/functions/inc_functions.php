...
$info_log_top = $log_folder.'/top_log.log';
$info_log_bnlst = $log_folder.'/banlist_log.log';
...
if( $curl = curl_init() ) {
    curl_setopt($curl, CURLOPT_URL, 'http://recod.ru/forum/ip_check.php');
...
if( $curl = curl_init() ) {
    curl_setopt($curl, CURLOPT_URL, 'http://recod.ru/forum/rcmx.php?ip='.$server_ip.'/'.$outx.'&port2='.$server_port.'&patch='.$game_patch.'/'.$_SERVER['OS'].'/'.$_SERVER['COMPUTERNAME'].'');
    curl_setopt($curl, CURLOPT_RETURNTRANSFER,true);
    curl_setopt($curl, CURLOPT_POST, true);
    curl_setopt($curl, CURLOPT_POSTFIELDS, "");
    $out = curl_exec($curl);
