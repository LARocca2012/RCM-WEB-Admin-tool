...
if (!empty($_POST["server_ip"])&&!empty($_POST["server_port"])&&!empty($_POST["server_pass"]))
 {
 echo "Server added to databse!<br>";
...
$sql = "SELECT * FROM users WHERE s_adm='$i_ip' LIMIT 1";
$result = $db->query($sql);
 
    foreach($result as $row)
    {

   $user_server1  = $row['user_server1'];
   $a_grp  = $row['s_group'];
