...
echo 'Error
<center><H1>Error!</H1>
<H1><a title="Settings" href="banlist.php">return BACK!</a></H1></center>';

exit;
}

  $deletee = trim($_GET["delgg"]);
 
 try
  {
    $db3i = new PDO('sqlite:'.$database_folder.'db2.sqlite');





//$nickr = $_SESSION['user_name'];
//$db3i->exec("INSERT INTO amnistia (playername1,ip1,guid1,reason1,time1,whooo1,patch1,whounban1) VALUES ('$pjhv','$country_name','','','$r','$kll','','$nickr')");
	   
//$db3i->query("DELETE FROM bans WHERE id='$deletee'");
 //
//
 $queryZn = $db3i->query("SELECT * FROM x_ranges WHERE id='$deletee' LIMIT 1");

    foreach($queryZn as $row)
    {
         
		$id = 	$row['id'];	
               $country_name = 	$row['ip_ranges'];
 
$db3i->Query("DELETE FROM x_ranges WHERE ip_ranges='$country_name'");
  $datetime = date('Y.m.d H:i:s');
AddToLog('['.$datetime.'] RANGE UNBAN: ' . $country_name . ' by ('.$hhh.')');
echo '
<br/><center><H1> UnBaned!</H1>
<H2><a title="Settings" href="ip_ranges.php">return BACK!</a></H2></center>'; 
}
